...
write('slip.html', page({
  ...
  body: `
      ...
      <div class="rule">A SLIP EXPERT&rsquo;S ACCOUNT</div>
      <div class="lede"><p>What follows is a longer, more technical account of SLIP, written for this site by a SLIP expert who has built a modern implementation of it. It goes deeper than the introduction above: the list structure as a graph, the physical cell, SLIP as an early API, portability, and what SLIP does and does not do for the programmer.</p></div>

      <h2>Introduction</h2>
      <p>Weizenbaum developed SLIP (CACM v6n9) in 1963 as a successor to his Knotted List Structures (KLS) of the previous year (CACM v5n3). In KLS the framework of what was to become SLIP was developed, significantly with a single list pointer rather than a doubly linked list. In SLIP we see the basic ideas extended and a fuller representation of the ideas developed for KLS.</p>
      ...
      <figure class="figure">
        ...
        <figcaption>Figure 1. SLIP DAG. Three SLIP lists in a DAG: the root list at the top and two subordinate lists below it. In 1966 this would be called a list of lists.</figcaption>
      </figure>
      ...
      <p>In a similar fashion a DAG represents a graph in which no node points to a parent node. Or if you will, a graph without cycles. Using the list of lists terminology the statements are tortuous. We will use the graphic terminology when talking about SLIP structures. And we will say that SLIP allows the representation of DAGs, a directed graph without cycles.</p>
      <p>Figure 2 shows a representation of the DAG from Figure 1, but here, the root node contents are made clear. The root node has two lists, an associative list and a standard list, and references to two leaf nodes. As a DAG, the leaf node&rsquo;s parent is the root node, and the root node has two siblings.</p>
      <figure class="figure" style="max-width:none">
        ...
        <figcaption>Figure 2. SLIP List Structure. The root node has two lists: an associative list with two &lt;key, value&gt; pairs, and a standard list with two data cells. The root points to the first and last data cell in the list, and to the first pair in the associative list; the terminal cells point back to the root. This is by design and makes best use of the available space in a SLIP cell.</figcaption>
      </figure>
      ...
      <figure class="figure">
        ...
        <figcaption>Figure 3. Physical SLIP Cell. Two physical words: the first holds two 15-bit addresses (previous and next cell) right-aligned, plus reserved bits for SLIP type information; the second holds user data.</figcaption>
      </figure>
