...
function nav(depth) {
  ...
  return `
      ...
      <nav class="site-nav" id="site-nav">
        ...
        <div class="nav-group">
          <a class="nav-top" href="${p}blog.html">Blog</a>
        </div>
        <div class="nav-group">
...
write('slip.html', page({
  ...
  body: `
      ...
      <h2>What SLIP is</h2>
      <p>SLIP stands for <strong>Symmetric LIst Processor</strong>. Weizenbaum designed it around 1962 and described it in the Communications of the ACM in 1963, when the list-processing ideas of IPL and LISP were still new. It is not a standalone language but a set of routines bolted onto a host language, first FORTRAN and then MAD. The version ELIZA used ran on MIT&rsquo;s CTSS on the IBM 7094: MAD for the logic, SLIP for the lists.</p>
      ...
      <figure class="figure" style="max-width:none">
        ...
        <figcaption>A SLIP cell: a datum held between a <span style="color:var(--eliza)">next</span> pointer forward and a <span style="color:var(--lamp-amber)">prev</span> pointer back.</figcaption>
      </figure>
      ...
      <ul>
        <li><code>SEQRDR</code> / <code>SEQLR</code>: make a <em>sequence reader</em> for a list, then read its next cell left to right. This is how ELIZA scans a list. A reader is what a modern language would call an iterator.</li>
        ...
        <li><code>HASH</code>, <code>YMATCH</code>, <code>ASSMBL</code>: hash a word into a table index, match a decomposition pattern, and assemble a reply from a template. These three sit in the SLIP listing but are not part of SLIP as published: they do ELIZA&rsquo;s work, and Weizenbaum appears to have kept them there for convenience rather than as an extension of the library.</li>
      ...
      <div class="callout"><span class="kicker">A living implementation</span><p>SLIP did not stay locked in 1963. Arthur Schwarz, a member of this project, has written <strong><a href="https://slipbits.com/gslip/">gSlip</a></strong>, a public-domain implementation in C++, which makes it possible to run and study SLIP code today rather than only read it.</p><p>It also carries SLIP into a modern idiom. It hides the mechanics of insertion and deletion, adds data types, and lets SLIP data be used directly as language primitives, so that <code>A = B + C</code> holds whether A, B and C are SLIP cells or ordinary integers, reals and strings.</p></div>

      <div class="rule">SLIP IN DETAIL</div>
      <div class="lede"><p>What follows is a longer, more technical account of SLIP, written for this site by Arthur Schwarz, who has built <a href="https://slipbits.com/gslip/">gSlip</a>, a modern implementation of it. It goes further than the introduction above: the list of lists restated as a graph, the physical cell, SLIP as an early API, portability, and what SLIP leaves to the programmer.</p></div>

      <h2>Introduction</h2>
      <p>Weizenbaum developed SLIP (CACM v6n9) in 1963 as a successor to his Knotted List Structures (KLS) of the previous year (CACM v5n3). In KLS the framework of what was to become SLIP was developed, significantly with a single list pointer rather than a doubly linked list. In SLIP we see the basic ideas extended and a fuller representation of the ideas developed than in KLS.</p>
      <p>In describing SLIP we talk about the contribution that SLIP made in the general, nascent, field of computer science. SLIP by itself was anticipated, but the unexpected nature of its contribution was in its construction as an Application Programming Interface (API) in which a set of functions are organized to perform some collective task, but the logic for the task is contained within the calling application. An API is not separately executable. With this we will give a short mention of portability within the framework of the time, and Weizenbaum&rsquo;s use of portability. Then there is the subject of SLIP, what it does and how it does it.</p>

      <h2>1.0 Glossary</h2>
      <p>There is an attempt to use current terminology when talking about SLIP objects. However, current terminology was not used in the SLIP defining article in the Communications of the ACM (CACM v6n9). So here is a glossary which stretches the gap between understanding the article and understanding the blog.</p>
      <dl class="glossary">
        <dt>address</dt><dd>Pointer to the first word of a SLIP object</dd>
        <dt>associative list</dt><dd>List of &lt;key, value&gt; pairs where both keys and values are SLIP Datum cells</dd>
        <dt>DAG</dt><dd>A Directed Acyclic Graph. A graph without cycles</dd>
        <dt>data</dt><dd>The data in a SLIPData cell. Either an integer or floating point number</dd>
        <dt>descriptor list</dt><dd>Associative list</dd>
        <dt>DG</dt><dd>A Directed Graph. A graph which may have cycles; a DAG is a type of DG</dd>
        <dt>LINKL</dt><dd>Pointer to the previous SLIP cell in a list</dd>
        <dt>LINKR</dt><dd>Pointer to the next SLIP cell in a list</dd>
        <dt>list</dt><dd>A SLIPHeader cell. A node on a graph</dd>
        <dt>Name</dt><dd>A reference to a SLIPHeader cell. A SLIPName cell</dd>
        <dt>node</dt><dd>A SLIPHeader cell. A node on a DAG</dd>
        <dt>reference</dt><dd>Pointer to the first word of a SLIP object</dd>
        <dt>root</dt><dd>A node which is not a child</dd>
        <dt>SLIPData</dt><dd>A SLIP cell containing data</dd>
        <dt>SLIPHeader</dt><dd>A SLIP cell defining the head of a list</dd>
        <dt>SLIPName</dt><dd>A SLIP cell containing a reference to another list (SLIPHeader)</dd>
        <dt>SLIPReader</dt><dd>A list iterator with memory</dd>
        <dt>SLIPSequencer</dt><dd>A list iterator without memory. A pointer to a SLIP cell</dd>
        <dt>Sublist</dt><dd>A list contained in a list, a.k.a. a child node in a graph</dd>
      </dl>

      <h2>2.0 SLIP Overview</h2>
      <p>In the literature SLIP was represented as a list of lists, where each list could have sublists. The representation is somewhat deceptive. It is better represented as a Directed Acyclic Graph (DAG) where each node on the graph has two lists, an associative list of key, value pairs, and a symmetric list of list atoms. In this context a list atom is a SLIP cell with a link to the previous SLIP cell and the next SLIP cell acting as a data carrier for a machine primitive (integer or single precision floating point), or a pointer to a contained list. Various Application Program Interface (API) functions supported the creation and deletion of list cells, and the modification of list cell data.</p>
      <figure class="figure">
        <svg class="slipfig" viewBox="0 0 300 180" role="img" aria-label="A SLIP DAG: a root node with two child nodes"><defs><marker id="slfa" markerWidth="9" markerHeight="9" refX="6.5" refY="3" orient="auto"><path d="M0,0 L6.5,3 L0,6 Z" fill="var(--lamp-amber)"/></marker></defs><circle cx="150" cy="34" r="17" fill="none" stroke="var(--lamp-amber)" stroke-width="2"/><circle cx="72" cy="140" r="17" fill="none" stroke="var(--eliza)" stroke-width="2"/><circle cx="228" cy="140" r="17" fill="none" stroke="var(--eliza)" stroke-width="2"/><line x1="139" y1="47" x2="84" y2="126" stroke="var(--lamp-amber)" stroke-width="2" marker-end="url(#slfa)"/><line x1="161" y1="47" x2="216" y2="126" stroke="var(--lamp-amber)" stroke-width="2" marker-end="url(#slfa)"/></svg>
        <figcaption>Figure 1. SLIP DAG.</figcaption>
      </figure>
      <p>Figure 1 shows three SLIP lists in a DAG. The root list is at the top and the two subordinate lists are below it. In 1966 this would be called a list of lists, with the subordinate nodes identified as lists, and at the time, all SLIP cells would be shown. The description of the structure would be an S-Expression, as in ( () () ), where the root is the outer parenthesis and each of the two contained lists are represented as ( ). Hence a list containing two lists.</p>
      <p>In SLIP, the root node is always a SLIPHeader data cell, and all other nodes are SLIPSublist cells. SLIPSublist cells are data cells that reference SLIPHeader cells. A SLIPHeader cell defines the list header node and references to the associative list and list.</p>
      <p>As a DAG the dialogue and representation is clearer than being represented as a list of lists. As a DAG we can say &lsquo;root&rsquo; with a clear and defined meaning, and we can say &lsquo;leaf&rsquo; and interior node. As a list of lists both terminology and representation become muddled. We could say the &lsquo;outermost list&rsquo; and then the &lsquo;list containing no sublist&rsquo; to represent the root and leaf, and then the &lsquo;a contained list containing sublists&rsquo; to represent an interior node. It is much clearer to reference the known statements about graphs rather than that of a list.</p>
      <p>In a similar fashion a DAG represents a graph in which no node points to a parent node. Or if you will, a graph without cycles. Using the list of lists terminology the statement is tortuous. We will use the graphic terminology when talking about SLIP structures. And we will say that SLIP allows the representation of DAGs, a directed graph without cycles.</p>
      <p>Figure 2 shows a representation of the DAG from Figure 1, but here, the root node contents are made clear. The root node has two lists, an associative list and a symmetric list, and references to two leaf nodes. As a DAG, the leaf node&rsquo;s parent is the root node, and the root node has two siblings.</p>
      <figure class="figure" style="max-width:none">
        <svg class="slipfig" viewBox="0 0 460 250" role="img" aria-label="SLIP list structure: a root node holding an associative list and a standard list, with two leaf nodes"><defs><marker id="slfb" markerWidth="9" markerHeight="9" refX="6.5" refY="3" orient="auto"><path d="M0,0 L6.5,3 L0,6 Z" fill="var(--lamp-amber)"/></marker></defs><circle cx="110" cy="120" r="18" fill="none" stroke="var(--eliza)" stroke-width="2"/><text x="110" y="124" text-anchor="middle" font-family="var(--mono)" font-size="10" fill="var(--ink-dim)">root</text><g stroke="var(--lamp-amber)" stroke-width="1.6" fill="none"><rect x="210" y="40" width="42" height="22"/><rect x="262" y="40" width="42" height="22"/><rect x="314" y="40" width="42" height="22"/><rect x="366" y="40" width="42" height="22"/></g><g fill="var(--ink-dim)" font-family="var(--mono)" font-size="9" text-anchor="middle"><text x="231" y="55">k</text><text x="283" y="55">v</text><text x="335" y="55">k</text><text x="387" y="55">v</text></g><g stroke="var(--lamp-amber)" stroke-width="1.6" fill="none"><rect x="234" y="150" width="42" height="22"/><rect x="286" y="150" width="42" height="22"/></g><g fill="var(--ink-dim)" font-family="var(--mono)" font-size="9" text-anchor="middle"><text x="255" y="165">d</text><text x="307" y="165">d</text></g><circle cx="70" cy="215" r="15" fill="none" stroke="var(--eliza)" stroke-width="2"/><circle cx="150" cy="215" r="15" fill="none" stroke="var(--eliza)" stroke-width="2"/><g stroke="var(--lamp-amber)" stroke-width="1.6"><line x1="126" y1="108" x2="208" y2="55" marker-end="url(#slfb)"/><line x1="252" y1="51" x2="260" y2="51" marker-end="url(#slfb)"/><line x1="304" y1="51" x2="312" y2="51" marker-end="url(#slfb)"/><line x1="356" y1="51" x2="364" y2="51" marker-end="url(#slfb)"/><path d="M408,51 C430,51 430,120 132,118" fill="none" marker-end="url(#slfb)"/><line x1="128" y1="118" x2="232" y2="160" marker-end="url(#slfb)"/><line x1="276" y1="161" x2="284" y2="161" marker-end="url(#slfb)"/><path d="M328,161 C350,161 350,124 130,122" fill="none" marker-end="url(#slfb)"/><line x1="100" y1="136" x2="76" y2="200" marker-end="url(#slfb)"/><line x1="118" y1="137" x2="146" y2="200" marker-end="url(#slfb)"/></g></svg>
        <figcaption>Figure 2. SLIP List Structure.</figcaption>
      </figure>
      <p>In Figure 2 the root node has two lists. An associative list with two &lt;key, value&gt; pairs, and a standard list with two data cells. The Figure shows that the root node points to the first and last data cell in the list, and points to the first &lt;key, value&gt; pair in the associative list, and the terminal data cell points to the root node, and the last value cell points to the root node. This is by design and makes best use of the available space in a SLIP cell.</p>

      <h2>2.1 Computer Architecture</h2>
      <p>The SLIP architecture is based on the computers available at that time. The ones used by Weizenbaum, the IBM 7094 and CDC 1604, accessed a full word of memory and had a maximum computer memory size of 32,768 words. An instruction could only access a word, and if address referencing using data in a word, only 15-bits were used. This meant that a 36-bit word, in the IBM 7094, and a 48-bit word, in the CDC 1604, had extra bits available for use, and that using these extra bits would not interfere with their use as an indirect reference to another word. Further, both machines could fit two addresses into a single word.</p>
      <p>A SLIP cell consists of two adjacent physical words of memory. The first (lowest addressed) word contains SLIP dependent information. The second (next physical address) contains user data.</p>
      <figure class="figure">
        <svg class="slipfig" viewBox="0 0 340 96" role="img" aria-label="A SLIP cell as two physical words: the first word holds two 15-bit addresses and reserved type bits, the second holds user data"><g stroke="var(--lamp-amber)" stroke-width="1.6" fill="none"><rect x="20" y="14" width="300" height="30"/><line x1="60" y1="14" x2="60" y2="44"/><line x1="190" y1="14" x2="190" y2="44"/><rect x="20" y="50" width="300" height="30"/></g><g fill="var(--ink-dim)" font-family="var(--mono)" font-size="10"><text x="40" y="33" text-anchor="middle">type</text><text x="125" y="33" text-anchor="middle">LINKL (15-bit)</text><text x="255" y="33" text-anchor="middle">LINKR (15-bit)</text><text x="170" y="69" text-anchor="middle">user data</text></g></svg>
        <figcaption>Figure 3. Physical SLIP Cell.</figcaption>
      </figure>
      <p>A SLIP cell uses two physical words of a computer. The first word has two addresses of 15-bits right aligned in a half word, plus two left aligned in a half word reserved areas for SLIP data type information.</p>

      <h2>2.2 SLIP Physical Architecture</h2>
      <p>The SLIP-centric use of a SLIP cell is to use the first word to contain the address of a previous SLIP cell in a list, and the address of the next SLIP cell in a list, and to use the reserved bits to carry SLIP cell type information.</p>
      <p>In CACM v6n9 the identified data types are:</p>
      <ul>
        <li><strong>0</strong>: SLIP Data cell. Interpret the second word in a SLIP cell as data.</li>
        <li><strong>1</strong>: SLIP Name. Interpret the second word in a SLIP cell as the address of a list, a.k.a. a SLIP Header cell.</li>
        <li><strong>2</strong>: SLIP Header, a list header. Interpret the second word in a SLIP cell as the address of the node associative list. The previous address is the address of the last SLIP cell in a list, and the next address is the first SLIP cell in a list.</li>
        <li><strong>3</strong>: SLIP Reader. The cell is a list iterator. Readers are standalone SLIP cells which are not contained in a list. Therefore both the first and second word of a SLIP cell are different than for a list element. A reader contains a pointer to the current SLIP cell and the current SLIP Header, the address of the previous SLIP Reader data, and the depth of the current SLIP Reader.</li>
      </ul>
      <p>What we have described is a SLIP Cell, data types 0&ndash;2, and an iterator, data type 3. Let&rsquo;s look at this in a little more detail.</p>
      <ol>
        <li>A NULL address has the value zero (0). Whenever an address has the value zero it is considered as not pointing at a SLIP cell object.</li>
        <li>An empty list is indicated by the SLIP Header cell pointing to itself in the first word; the previous and next address is the address of the SLIP Header cell. A SLIP Header never has either the previous or next address NULL.</li>
        <li>An associative list is not required. A SLIP Header cell without an associative list has the associative list address, the second word of a SLIP cell, equal to NULL.</li>
        <li>Data. Data can be either an integer or floating point data. The application is required to know which is contained within a SLIP cell.</li>
      </ol>

      <h2>3.0 SLIP as an API</h2>
      <p>APIs have several definitions. We will use the definition that an API is a set of functions supporting a single task and which must be included in another program and which does not share a memory/data space with the caller. Like a sin function, an API does nothing without an externally supplied program calling it. This definition excludes a library of functions in which each member of the library is separately callable but which are independent in functionality from other library functions. This excludes, for example, a scientific library of functions in which there are sine, cosine and tangent functions.</p>
      <p>SLIP was probably the first API. SLIP provided a complete and encapsulated set of functions whose sole purpose was to support the construction and use of a DAG. Any FORTRAN II program requiring the use of a DAG could use SLIP (subject to portability issues).</p>

      <h2>4.0 SLIP Portability</h2>
      <p>Portability of an API was constrained by the computer software language and the computer architecture. If one or the other was inconsistent in supporting portability, then the API was unportable. SLIP was &lsquo;ported&rsquo; from an IBM 7094 to a CDC 1604 and written in the FORTRAN II programming language. What does this mean?</p>
      <p>The language does not support bit operations, and certain other pointer operations. In order to use SLIP at all, these required language elements must be hard coded in the assembly language of the respective machines. The remainder of the functionality, up to some hardware caveats, was portable. Compiling SLIP using the IBM 7094 or CDC 1604 compilers would yield compatible functionality. Writing appropriate missing language functionality in assembly language would mean that all SLIP functionality was available for use.</p>
      <p>The computer architectures supported creation of a SLIP cell; that is, it was possible to use two computer words to contain all required fields to enable SLIP processing.</p>
      <p>The computer architectures did not support binary compatibility. This meant that binary data output from one computer could not be input and used on the other. This is briefly mentioned in the CACM article when portability is addressed. The reasons for this are that on computers of the day both integer and floating point number internal formats were different on different computers. Floating points were all different; there was no standardized format. Integer formats were better quantified but they could be anything from one&rsquo;s complement, two&rsquo;s complement, sign-magnitude, fixed point and decimal. On the IBM and CDC computers, the floating points were different and the integer formats were different, sign-magnitude on the IBM and one&rsquo;s complement on the CDC.</p>
      <p>It could be possible to generalize functionality and create a SLIP standard format for both floating point and integer, but this was not done. Indeed, the process for doing this was difficult and meant that for each hosting computer a separate package needed to be included to convert internal number formats to some standardized format. This would reduce portability.</p>
      <p>SLIP was portable up to supporting SLIP functionality, with the addition of assembly-language functions to accommodate language &lsquo;failures&rsquo;.</p>

      <h2>5.0 SLIP Functionality</h2>
      <p>Well. What does it do? We&rsquo;re going to take you through some superficial details of the workings of SLIP. And we&rsquo;re going to do this through the lens of the current ideas of software and software functionality, not to criticize the product, but to identify some weak points. This is not a user&rsquo;s manual so the details of operations and the functions required to support them should be looked for in the CACM v6n9 article.</p>
      <p>SLIP in general doesn&rsquo;t do much for you. The application has to know the address of SLIP cells and has to know the types of data contained in SLIP cells. We can&rsquo;t take this as criticism because SLIP does everything requested of it: it allows the creation of lists and DAGs, and allows iteration through them. This is what is wanted. But it makes life harder for the application by requiring the application to know the address of SLIP cells so that the included functionality can do the mechanics of manipulation required. One wonders why Weizenbaum didn&rsquo;t see this as a defect and take on the labor of providing functionality &lsquo;hiding&rsquo; the mechanics of implementation from the application by &ldquo;doing it for you&rdquo;. But Weizenbaum was a creator, an innovator at a time when there was nothing like this, and no guidelines as to what to do. He did well, but I wish the mechanics were better hidden.</p>
      <p>FORTRAN II has one strong liability where SLIP is concerned. FORTRAN II owned memory, and there was no facility for an application to acquire memory from some memory resource. In languages such as C and C++ they support acquisition of memory from the &lsquo;heap&rsquo; using &lsquo;new&rsquo; to acquire such memory, and &lsquo;delete&rsquo; to, well, dump it when no longer needed. This did not exist in FORTRAN II (or FORTRAN IV/66, or FORTRAN 77, or FORTRAN 90). So the application had to allocate its own memory and to tell SLIP where it is. This enabled SLIP to do its own memory management, to acquire memory and delete memory to support the transitive nature of memory usage. As a note, no other procedural language at this time had a concept of transitory use of memory. This didn&rsquo;t occur until C, some 7 or 8 years hence. And to those who are interested, LISP is not a procedural language.</p>
      <p>Memory use was cooperative. An application needing a SLIP cell had to ask for it, and when it was no longer needed, had to release it. This included individual SLIPData (and the SLIPReader), and lists. At the time this would have been completely novel. FORTRAN did not have dynamic memory; once memory was allocated, it existed forever. The notion of a stack frame where locals (local variables in a function) were automatically created on function call and deleted on function exit was not known. And yet, with the artifice of the application creating a block of memory and passing it to SLIP, SLIP achieved dynamic memory usage.</p>
      <p>A list or a DAG is a complex structure. SLIP provided two mechanisms to iterate across the structure. One was called a Sequencer, the other a Reader (SLIPReader). The Sequencer was a simple SLIP cell pointer which &lsquo;knew&rsquo; how to traverse to the next or previous cell in a DAG. But it had no memory. This meant that traversal to a child node was one way; there was no way back to the parent. A Reader had memory. It could enter into a child node and return to the parent. But memory takes resources and with 32,768 words of memory for data and instructions, there was some caution in using a Reader.</p>
      <p>The Sequencers and Readers are search functions. In a list there are three types of cells, the list header (SLIPHeader), the list data (SLIPData) and a child reference (SLIPSublist). You can search for the SLIPData and SLIPSublist cells. And note, with a Reader the search terminates on the root SLIPHeader cell, and with the Sequencer the search terminates on the SLIPHeader cell of the current list being searched (either the child or parent list). That means that with a Reader you can automatically go down and up a DAG, but with a Sequencer you can go down and cannot go up.</p>
      <p>There is also a capability to implement a form of recursion (the SLIP VISIT function). By current standards it is very crude. A lot of work has to be done by the application in setting up and using what we call now a stack frame, and since true recursion is not available in FORTRAN, recursion becomes an iterative loop where each loop cycle requires attention to the created stack frame.</p>
      <p>The data in a SLIPData cell could be retrieved by the application, but the application was required to know whether the data was an integer or a floating point number. The acquisition routine used depended on this knowledge and would return a value of the indicated type. In actuality, the access routines (INHALT, CONT) took the data in the data word and returned it as either an integer or a floating point number.</p>
      ...
      <h2>8.0 Note</h2>
      <p>For those interested in pursuing a modern version of SLIP, there is a pre-production version at <a href="https://slipbits.com/gslip/">https://slipbits.com/gslip/</a>. This is a modern version written in C++. It removes the mechanics of insertion/deletion, allows for several data types and the direct use of SLIPData cells in computations plus some other things. A user manual, source code, and a Win1x (x86-64) executable is provided.</p>
...
write('talks.html', page({
  ...
  body: `
      ...
      <ul class="talklist">
        <li><span class="who">Berry, D. M.</span><span class="ttl">&ldquo;Inventing ELIZA: How the First Chatbot Shaped the Future of AI&rdquo;</span><span class="meta">Book talk, with respondents Michael Jonik and Moinul Zaber, chaired by Tanya Kant, Sussex Digital Humanities Lab, University of Sussex &middot; 15 October 2026</span></li>
        ...
        <li><span class="who">Weil, P.</span><span class="ttl">&ldquo;Looking Back on MrMind&rdquo;</span><span class="meta"><a href="https://calendar.usc.edu/event/retro-ai-symposium">Retro AI: Archaeologies of A.I. symposium</a>, USC, USA &middot; 31 July 2026</span></li>
        <li><span class="who">Weil, P., Marino, M. C. &amp; Schwarz, A. I.</span><span class="ttl">&ldquo;Inventing ELIZA&rdquo;</span><span class="meta">Public conversation, moderated by Claire L. Evans, <a href="https://wilshireonline.net">Wilshire Online</a>, Los Angeles &middot; 19 July 2026</span>
          <div class="talk-shots">
            <figure><a target="_blank" rel="noopener" href="assets/images/wilshire-poster.jpg"><img src="assets/images/wilshire-poster.jpg" alt="Poster for Inventing ELIZA at Wilshire Online, 19 July 2026" loading="lazy"></a><figcaption>The poster, set in the DOCTOR listing.</figcaption></figure>
            <figure><a target="_blank" rel="noopener" href="assets/images/wilshire-panel.jpg"><img src="assets/images/wilshire-panel.jpg" alt="Claire L. Evans seated with a microphone; Peggy Weil lying on the couch reading" loading="lazy"></a><figcaption>Claire L. Evans in conversation with Peggy Weil, who took the couch.</figcaption></figure>
            <figure><a target="_blank" rel="noopener" href="assets/images/wilshire-group.jpg"><img src="assets/images/wilshire-group.jpg" alt="Mark C. Marino, Claire L. Evans, Peggy Weil and Arthur I. Schwarz after the event" loading="lazy"></a><figcaption>Mark C. Marino, Claire L. Evans, Peggy Weil and Arthur I. Schwarz.</figcaption></figure>
          </div>
        </li>
        <li><span class="who">Marino, M. C., Berry, D. M., Shrager, J., Schwarz, A. I., Weil, P., Ciston, S., Hay, A. C. &amp; Millican, P.</span><span class="ttl">&ldquo;Inventing ELIZA: an exploration of the first chatbot&rdquo;</span><span class="meta">Electronic Literature Organization (ELO) 2026, online &middot; 17 July 2026</span></li>
      ...
      <ul class="talklist">
        ...
        <li><span class="who">Berry, D. M., Ciston, S., Hay, A. C., Marino, M. C., Millican, P., Schwarz, A. I., Shrager, J. &amp; Weil, P.</span><span class="ttl">&ldquo;Getting to know ELIZA: ChatGPT&rsquo;s Great Grandmother&rdquo;</span><span class="meta">MIT &middot; 11 October 2023</span></li>
      </ul>
...
write('book.html', page({
  ...
  body: `
      ...
      <div class="lede"><p>How the First Chatbot Shaped the Future of AI. A study by Sarah Ciston, David M. Berry, Anthony C. Hay, Mark C. Marino, Peter Millican, Jeff Shrager, Arthur I. Schwarz and Peggy Weil, built on the recovered source code, published by MIT Press in the Software Studies series.</p></div>
      ...
      <div class="cover-note muted">
        <span class="kicker">MIT Press &middot; Software Studies</span>
        <h2>Inventing ELIZA</h2>
        <p>Inventing ELIZA: How the First Chatbot Shaped the Future of AI. By Sarah Ciston, David M. Berry, Anthony C. Hay, Mark C. Marino, Peter Millican, Jeff Shrager, Arthur I. Schwarz and Peggy Weil.</p>
